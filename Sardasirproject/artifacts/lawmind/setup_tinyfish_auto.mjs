...
async function setup() {
  try {
    ...
    if (checkError && checkError.code !== 'PGRST116') {
      console.error('❌ Error checking configuration:', checkError.message);
      console.log('\n⚠️  The database constraint may not be updated yet.');
    ...
    if (existing) {
      console.log('✅ TinyFish row already exists');
      
      if (existing.is_active) {
        console.log('✅ TinyFish is already enabled');
        console.log('\n🎉 Setup already complete! Nothing to do.');
        return;
      }

      console.log('⚠️  TinyFish exists but is disabled. Updating...');
    } else {
      console.log('❌ TinyFish row does not exist');
      console.log('\n📝 Step 2: Inserting TinyFish provider...');
      ...
      if (insertError) {
        if (insertError.message.includes('violates check constraint')) {
          console.error('❌ Database constraint needs updating');
        ...
        console.error('❌ Error inserting TinyFish:', insertError.message);
        return;
      }

      console.log('✅ TinyFish provider inserted successfully');
    }

    // Step 3: Update API key and enable
    if (existing && !existing.is_active) {
      console.log('\n📝 Step 3: Enabling TinyFish and adding API key...');
      ...
      console.log('✅ TinyFish enabled and API key added');
    }
    ...
    console.log('\n✅ Setup Complete!\n');
    console.log('📊 TinyFish Configuration:');
    console.log('   Provider:', verify.provider);
    console.log('   Model:', verify.model);
    console.log('   Base URL:', verify.base_url);
    console.log('   Status:', verify.is_active ? '✅ ACTIVE' : '❌ INACTIVE');
    
    console.log('\n🎉 TinyFish is now ready to use!');
    console.log('\n📍 Next Steps:');
    console.log('   1. Go to AI Agent page in LawMind');
    console.log('   2. Click "🌐 Web Research" button (top-right, teal colored)');
